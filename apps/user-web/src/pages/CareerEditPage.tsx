...
function CareerEditPage() {
  ...
  const handleComplete = async (submission: CareerEditSubmission) => {
    ...
    await updateCertificate(
      ...
      {
        name: applicant.name,
        birth_date: toIsoDate(applicant.birthDate),
        gender: toApiGender(applicant.gender),
        job_title: record.position,
        key_responsibilities: record.duties,
        hire_date: toIsoDate(record.startDate),
        expiration_date: original.expirationDate,
        retirement_date: toIsoDate(record.endDate),
